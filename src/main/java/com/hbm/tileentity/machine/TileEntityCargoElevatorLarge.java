...
public class TileEntityCargoElevatorLarge extends TileEntityLoadedBase {
	...
	public static final double speed = 1.5D / 20D; // 2 blocks per second
	...
	public void updateEntity() {
		...
		if(!worldObj.isRemote) {
			...
			if(worldObj.getBlock(xCoord, yCoord - 1, zCoord) == ModBlocks.cargo_elevator_large) {
				...
				if(pos != null && pos[0] == xCoord && pos[2] == zCoord) {
					TileEntityCargoElevatorLarge lower = (TileEntityCargoElevatorLarge) worldObj.getTileEntity(pos[0], pos[1], pos[2]);
		...
		if(this.extension != this.prevExtension) {
			...
			List<Entity> toLift = worldObj.getEntitiesWithinAABB(Entity.class, AxisAlignedBB.getBoundingBox(xCoord - 1.99, liftLower, zCoord - 1.99, xCoord + 2.99, liftUpper, zCoord + 2.99));
